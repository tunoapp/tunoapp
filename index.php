...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = $pdo->prepare("SELECT * FROM users WHERE (username = :usernameOrEmail OR mail = :usernameOrEmail) AND active = 1");
    ...
    if ($user && password_verify($password, $user['passHash'])) {
        ...
        $updateQuery = $pdo->prepare("UPDATE users SET lastSignIn = NOW() WHERE iduser = :id");
        $updateQuery->execute(['id' => $user['iduser']]);
...
<body class="align">
  <div class="grid">
    ...
    <div class="img-container">
        <img src="./img/Logo.png" class="text--center" alt="">
    </div>
    ...
    <form method="POST" class="form login">
      ...
      <div class="form__field">
        <label for="username_email"><svg class="icon">
            <use xlink:href="#icon-user"></use>
          </svg><span class="hidden">Usuari o correu electrònic</span></label>
        <input autocomplete="username" id="username_email" type="text" name="username_email" class="form__input" placeholder="Usuari o correu electrònic" required>
      </div>
